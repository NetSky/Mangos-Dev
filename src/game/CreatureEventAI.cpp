...
void CreatureEventAI::ProcessAction(CreatureEventAI_Action const& action, uint32 rnd, uint32 EventId, Unit* pActionInvoker)
{
    switch (action.type)
    {
        ...
        case ACTION_T_SUMMON_GAMEOBJECT:
        {
            ...
            if (i == sEventAIMgr.GetCreatureEventAISummonMap().end())
            {
                sLog.outErrorDb( "CreatureEventAI: failed to spawn Gameobject %u. Summon map index %u does not exist. EventID %d. GameobjectID  %d",action.summon_gameobject.gameobjectId, action.summon_gameobejct.spawnId, EventId, m_creature->GetEntry());
                return;
            }
            
            GameObject* summon_go;
            summon_go = m_creature->SummonGameobject(action.summon_gameobject.gameobjectId, (*i).second.position_x, (*i).second.position_y, (*i).second.position_z, (*i).second.orientation, (*i).second.SpawnTimeSecs*1000);
            if (!summon_go)
                sLog.outErrorDb( "CreatureEventAI: failed to spawn Gameobject %u. EventID %d. GameobjectID  %d",action.summon_gameobject.gameobjectId, action.summon_gameobejct.spawnId, m_creature->GetEntry());
